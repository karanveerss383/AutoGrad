...
Value* init_weights(size_t type, size_t dist, int fan_in, int fan_out){
  
  Value* weights = malloc(sizeof(Value) * fan_in * fan_out);
  ...
  for (int i = 0; i < fan_out; i++){
    ...
    for (int j = 0; j < fan_in; j++){
      ...
      weights[i * fan_in + j].data = ((float)rand() / RAND_MAX) * (dist * std_dev) - ((dist - 1) * std_dev);
      weights[i * fan_in + j].grad = 0.0f;
